    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> ManageUserClaims(string userId)
        {
            ...
            foreach (Claim claim in ClaimsStore.AllClaims)
            {
                ...
                {
                    ClaimStyle = claim.Type
                };
                /* if user has the claims, set isSelected propperty to true,
                so the checkbox next to claim is selected on UI
                 */
        ...
        public async Task<IActionResult> ManageUserClaims(UserClaimsViewModel model, string userId)
        {
            ...
            result = await userManager.AddClaimsAsync(user,
                model.Claims.Select(y => new Claim( y.ClaimStyle, y.IsSelected ? "true":"false")));
            if (!result.Succeeded)
            {
...
//        [Authorize(Policy = "EditRolePolicy")]
        ...
        public async Task<IActionResult> EditUserInRole(string roleId)
        {
            ...
            if(role == null)
            {
                ViewBag.ErrorMessage = $"Role Id '{roleId}' not be found";
                return View("NotFound");
            }
        ...
        public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            else
            {
                ...
                catch (DbUpdateException ex)
                {
                    ...
                    @ViewBag.ErrorMessage = $"'{role.Name}' role cannot be deleted as there are users in role. " +
                        $"If you want to delete this role, pls remove the users from the role and try to delete. ♥";
                    return View("Error");
                }
            }
        }
